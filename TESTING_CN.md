...
- ✅ Schema 配置正确性
- ✅ 初始化脚本可执行性
- ✅ **E2E/契约测试**在工作流任务中的体现（见下方说明）

**预期结果**：所有 17 项检查都应该通过 ✅

### E2E / 契约测试在哪里？

本模板是**工作流模板**，不包含具体业务的 e2e 测试代码；E2E 和契约测试体现在**任务列表**里，由你在具体项目中执行：

| 位置 | 说明 |
|------|------|
| **Task 4.2**（Phase 4, Step 8） | **后端契约测试**：骨架 API 完成后，验证响应格式符合 `spec.md`、StandardResp、错误码与 trace_id。 |
| **Task 5.6**（Phase 5, Step 10） | **前后端集成 / E2E**：前后端联调后，运行集成测试，覆盖完整用户流程（end-to-end）。 |

运行 `./validate.sh` 时，**Check 17** 会检查 `tasks.md` 中是否包含 “Contract Testing”或“end-to-end”/“E2E” 的表述，通过即表示工作流中已包含 E2E/契约测试任务。具体项目的 e2e 用例（如 Playwright/Cypress）需在初始化后的项目里按 Task 4.2 / Task 5.6 自行添加。
